...
while ( <STDIN> ) {
	if ( m/^   ([0-9A-F][0-9A-F][0-9A-F][0-9A-F]) (A2|30) B0 (  |0C)\s+(\[\d+\]\s+)?\d+[^;]+_TDO.*?$/ ) {
		push(@tdoList, 1 + hex($1));
	}
	if ( m/^   ([0-9A-F][0-9A-F][0-9A-F][0-9A-F]) 92 B1\s+(\[\d+\]\s+)?\d+[^;]+_TDI.*?$/ ) {
		push(@tdiList, 1 + hex($1));
	}
	if ( m/^   ([0-9A-F][0-9A-F][0-9A-F][0-9A-F]) [9D]2 B2\s+(\[\d+\]\s+)?\d+[^;]+_TMS.*?$/ ) {
		push(@tmsList, 1 + hex($1));
	}
	if ( m/^   ([0-9A-F][0-9A-F][0-9A-F][0-9A-F]) [CD]2 B3\s+(\[\d+\]\s+)?\d+[^;]+_TCK.*?$/ ) {
