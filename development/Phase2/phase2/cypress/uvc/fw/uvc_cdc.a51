...
db	04H; 4 bNumInterfaces 1 Number of interfaces in the configuration (will be chaneged when CDC is merged in it ) at the moment 2 interfaces 1 control and 1 streaming
...
db     6CH 
db     0DCH 
db     02H  ; dwClockFrequency 	4 	Number 	Use of this field has been deprecated. The device clock frequency in Hz. This will specify the units used for the time information fields in the Video 
...
db	04H; 8 bTerminalLink 1 Constant The terminal ID of the Output Terminal to which the video endpoint of this interface is connected.
...
db    0B0H
db    00H               ;       /* Width of the frame : 176 */
db    90H
db    00H               ;       /* Height of the frame : 144 */
db    00H
db    0C0H
db    5DH
...
db	00H; 8 iFunction 1 Index of string descriptor for the function
      ...
      db   04H     ;; ",                 -- bDescriptorType = interface descriptor
      db   02H     ;; ",                 -- bInterfaceNumber = 0
      db   00H     ;; ",                 -- bAlternateSetting = 0
      db   01H     ;; ",                 -- bNumEndpoints = 1
      db   02H     ;; ",                 -- bInterfaceClass = Communication Interface
      db   02H     ;; ",                 -- bInterfaceSubClass = Abstract Control Model
      db   01H     ;; ",                 -- bInterfaceProtocol = V.25ter (required for Linux CDC-ACM driver)
      db   00H     ;; ",                 -- iInterface = none

       ; -- 5 bytes functional descriptor (header)
      db   05H     ;; ",                 -- bLength = 5 bytes
      db   24H     ;; ",                 -- bDescriptorType = CS_INTERFACE
      db   00H     ;; ",                 -- bDescriptorSubtype = header
      db   10H; ", 
	  db   01H     ;; ",          -- bcdCDC = 1.10
      ;  -- 4 bytes functional descriptor (abstract control management)
      db   04H     ;; ",                 -- bLength = 4 bytes
      db   24H     ;; ",                 -- bDescriptorType = CS_INTERFACE
      db   02H     ;; ",                 -- bDescriptorSubtype = Abstract Control Mgmnt
      db   00H     ;; ",                 -- bmCapabilities = none
     ;   -- 5 bytes functional descriptor (union)
      db   05H     ;; ",                 -- bLength = 5 bytes
      db   24H     ;; ",                 -- bDescriptorType = CS_INTERFACE
      db   06H     ;; ",                 -- bDescriptorSubtype = union
      db   02H     ;; ",                 -- bMasterInterface = 0
      db   03H     ;; ",                 -- bSlaveInterface0 = 1
      ;  -- 5 bytes functional descriptor (call management)
      db   05H     ;; ",                 -- bLength = 5 bytes
      db   24H     ;; ",                 -- bDescriptorType = CS_INTERFACE
      db   01H     ;; ",                 -- bDescriptorSubType = Call Management
      db   00H     ;; ",                 -- bmCapabilities = no call mgmnt
      db   01H     ;; ",                 -- bDataInterface = 1
      ;  -- 7 bytes endpoint descriptor (notify IN)
      db   07H     ;; ",                 -- bLength = 7 bytes
      db   05H     ;; ",                 -- bDescriptorType = endpoint descriptor
      db   82H     ;; ",                 -- bEndpointAddress = IN 2
      db   03H     ;; ",                 -- bmAttributes = interrupt data
      db   08H; ", 
	  db   00H     ;; ",          -- wMaxPacketSize = 8 bytes
      db   0fH     ;; ",                 -- bInterval = 2**14 frames
     ;   -- 9 bytes interface descriptor (data class)

      db   09H     ;; ",                 -- bLength = 9 bytes
      db   04H     ;; ",                 -- bDescriptorType = interface descriptor
      db   03H     ;; ",                 -- bInterfaceNumber = 1
      db   00H     ;; ",                 -- bAlternateSetting = 0
      db   02H     ;; ",                 -- bNumEndpoints = 2
      db   0aH     ;; ",                 -- bInterfaceClass = Data Interface
      db   00H     ;; ",                 -- bInterfaceSubClass = none
      db   01H     ;; ",                 -- bInterafceProtocol = none
      db   00H     ;; ",                 -- iInterface = none
	  
;	-- 7 bytes endpoint descriptor (data IN)
      db   07H     ;; ",                 -- bLength = 7 bytes
      db   05H     ;; ",                 -- bDescriptorType = endpoint descriptor
      db   81H     ;; ",                 -- bEndpointAddress = IN 1
      db   02H     ;; ",                 -- bmAttributes = bulk data
      db   00H; ", 
	  db   02H     ;; ",          -- wMaxPacketSize = 512 bytes
      db   00H     ;; ",                 -- bInterval
    ;    -- 7 bytes endpoint descriptor (data OUT)
      db   07H     ;; ",                 -- bLength = 7 bytes
      db   05H     ;; ",                 -- bDescriptorType = endpoint descriptor
      db   01H     ;; ",                 -- bEndpointAddress = OUT 1
      db   02H     ;; ",                 -- bmAttributes = bulk data
      db   00H  ; ", 
	  db   02H     ;; ",          -- wMaxPacketSize = 512 bytes
      db   00H     ;; ",                 -- bInterval = never NAK
