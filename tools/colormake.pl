...
#	}
	...
	elsif ($thisline =~ /^(\*\s+)([\w\s]+)(\s+\*)/)
	{
		$in = $1;
		$thisline = "$col_brighten$1$col_gcc$2$col_norm$col_default$col_brighten$3\n";
	}
	elsif ($in eq 'gcc')
	{
	...
	{
		$thisline =~ s|(INFO):([^:]+):([^ ]+) - (.*)$|$1:$col_yellow$2$col_norm$col_default:$col_cyan$3$col_norm$col_default - $col_comment$4|;
	}
	elsif ($thisline =~ /^WARNING:/)
	{
		$thisline =~ s|(WARNING):([^:]+):([^ ]+) - (.*)$|$1:$col_yellow$2$col_norm$col_default:$col_cyan$3$col_norm$col_default - $col_warning$4|;
	}
	elsif ($thisline =~ /^ERROR:/)
	{
		$thisline =~ s|(ERROR):([^:]+):([^ ]+) - (.*)$|$1:$col_yellow$2$col_norm$col_default:$col_cyan$3$col_norm$col_default - $col_error$4|;
	}
